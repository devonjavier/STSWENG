...
type ShoppingCartContext = {
    ...
    increaseCartQuantity: (id: number, appointmentId: number) => Promise<void>;
    decreaseCartQuantity: (id: number) => void;
    removeFromCart: (id: number) => void;
    cartQuantity: number;
    cartItems: CartItemProps[];
    addToCart: (appointmentId: number, itemId: number, quantity: number) => Promise<void>; // NEW: Backend API call to add an item to the cart.
  };
  ...
  const ShoppingCartProvider = ({ children }: ShoppingCartProviderProps) => {
    ...
    // NEW: Add an item to the cart in the backend.
    async function addToCart(appointmentId: number, itemId: number, quantity: number) {
        try {
    ...
    async function increaseCartQuantity(id: number, appointmentId: number) {
      const storeItem = storeItems.find((item) => item.itemid === id); // Find the item in store items.
      ...
      if (currentQuantityInCart < availableQuantity) {
        try {
          const quantityToAdd = availableQuantity - currentQuantityInCart;

          await addToCart(appointmentId, id, quantityToAdd); // Call the backend to add the item.
...
const Page = ({ searchParams }: { searchParams: { service: string, serviceType: string, maincustomerfirstname: string, maincustomermiddlename: string, maincustomerlastname: string, phonenumber: string, emailaddress: string, needsparking: string, additionalrequests: string, hours: number, additionalpackage: string } }) => {
  ...
  const StoreItem = ({ itemid, itemname, price, imageName, description, quantity, appointmentId }: StoreItemProps) => {
      ...
      <div className="max-w-xs h-full bg-gray-100 shadow-md rounded-lg overflow-hidden flex flex-col"> {/* Card container */}
        ...
        <div className="mt-auto flex items-center justify-center">
            ...
            <button
              ...
              onClick={() => increaseCartQuantity(itemid, appointmentId)} // Pass `itemid` and `appointmentId` here.
            >
              Available
            </button>
          ) : (
